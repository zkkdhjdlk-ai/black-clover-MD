const handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  // 🔹 Asegurar que exista el objeto del usuario
  if (!global.db.data.users[who]) {
    global.db.data.users[who] = { warn: 0 }; // inicializamos warn
  }
  ...
  // 🔹 Aseguramos que warn sea un número
  ...
  if (user.warn >= 3) {
    if (!bot.restrict) {
      return m.reply(
        '*[❗] 𝙴𝙻 𝙿𝚁𝙾𝙿𝙸𝙴𝚃𝙰𝙳𝙾 𝙳𝙴𝙻 𝙱𝙾𝚃 𝙽𝙾 𝚃𝙸𝙴𝙽𝙴 𝙷𝙰𝙱𝙸𝙻𝙸𝚃𝙰𝙳𝙾 𝙻𝙰𝚂 𝚁𝙴𝚂𝚃𝚁𝙸𝙲𝙲𝙸𝙾𝙽𝙴𝚂 (#enable restrict) 𝙲𝙾𝙽𝚃𝙰𝙲𝚃𝙴 𝙲𝙾𝙽 𝙴𝙻 𝙿𝙰𝚁𝙰 𝚀𝚄𝙴 𝙻𝙾 𝙷𝙰𝙱𝙸𝙻𝙸𝚃𝙴*',
      );
    }
    user.warn = 0;
    await m.reply(
