...
global.dfail = (type, m, conn) => {
  ...
  if (!conn) {
    console.warn('⚠️ [dfail] Conexión (conn) no definida.')
    return
  }

  if (!m || !m.chat) {
    console.warn('⚠️ [dfail] Objeto mensaje (m) no válido.')
    return
  }

  if (msg) {
    return conn.reply(m.chat, msg, m).then(() => m.react('🛑')).catch(console.error)
  }
}