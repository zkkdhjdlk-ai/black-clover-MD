...
opcion = await question(`${lineM}  
 ...
 ${chalk.whiteBright('╭┄┈✞┈┄┈✯┈┄✞┈┄┈✯┄┈✞┈┄')}     
 ${chalk.whiteBright('┊')} ${chalk.italic.magenta("Seleciona el numero de")}
 ${chalk.whiteBright('┊')} ${chalk.italic.magenta("La opcion para conectarse")}
 ${chalk.whiteBright('╰┄┈✞┈┄┈✯┈┄✞┈┄┈✯┄┈✞┈┄')} 
  ${chalk.italic.red(`El mejor bot 🥷`)}
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "blackJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
console.log(chalk.bold.yellow(`\n⚔️ ESCANEA EL CÓDIGO QR EXPIRA EN 45 SEGUNDOS`))}
}
if (connection == 'open') {
...
let prekey = []
let directorio = readdirSync("./blackSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./blackSession/${files}`)
})
} 
...
const directories = ['./blackSession/', './blackJadiBot/']
...
setInterval(async () => {
  ...
  const bio = `⚔️ Black-clover-MD|「🕒」Aᥴ𝗍і᥎o: ${uptime}`;
